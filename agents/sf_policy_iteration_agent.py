...
class SFPolicyIterationAgent(PolicyIterationAgent):
    ...
    def greedy_action(self, state: int) -> Tuple[float, int]:
        ...
        max_indices = np.where(self.Q[state] == np.max(self.Q[state]))[0]
        for action in range(self.env.n_actions):
            if abs(self.Q[state][action] - max_value) < 1e-12:
    ...
    def calculate_action_transition(self, state: int, input_action: int) -> float:
        ...
        for action in self.env.actions:
            ...
            for next_action in self.env.actions:
                sub_weighted_sum += self.pi[next_state_encoded, next_action] * self.psi[next_state_encoded, next_action]
            weighted_sum += probabilities[action] * (phi + self.gamma * sub_weighted_sum)
        return weighted_sum
            ...
            for state in range(self.env.n_states):
                for action in range(self.env.n_actions):
                    if not self.infinite_horizon and self.env.is_terminal_state(self.env.decode_state(state)):
                        ...
                        prev_value = copy.deepcopy(self.psi[state, action])
