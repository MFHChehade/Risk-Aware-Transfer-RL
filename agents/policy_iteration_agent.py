...
class PolicyIterationAgent(ValueBasedAgent):
    ...
    def greedy_action(self, state: int) -> Tuple[float, int]:
        ...
        max_value = np.max(transition_values)
        max_indices = np.where(transition_values == max_value)[0]
        for action in range(self.env.n_actions):
            if abs(transition_values[action] - max_value) < 1e-12:
                if action not in max_indices:
                    max_indices = np.append(max_indices, action)
    ...
    def train(self) -> None:
        ...
        while not policy_stable:
            ...
            for state in range(self.env.n_states):
                ...
                _, best_action_indices = self.greedy_action(state)
                new_pi = np.zeros(self.env.n_actions)
                new_pi[best_action_indices] = 1 / len(best_action_indices)
                if not np.array_equal(self.pi[state], new_pi):
                    policy_stable = False
                    self.pi[state] = new_pi
        # set policy for terminal states
        for state in range(self.env.n_states):
            if self.env.is_terminal_state(self.env.decode_state(state)):
